...
void flash_eprom(const char *filename);
...
uint32_t getfileCRC(const char *name) {
    ...
    while (size = fread(buffer, 1, PAGESIZE, file)) {
        ...
        for(int n = 0; n < size; n++) crc.add(buffer[n]);
...
void flash_eprom(const char *filename) {
    ...
    uint32_t size = 0;
    uint32_t read_moscrc = 0;
    uint32_t expected_moscrc = 0;
